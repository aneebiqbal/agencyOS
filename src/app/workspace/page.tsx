...
const DEMO_STEPS = [
  {
    ...
    detail: "Add or review pipeline entries and qualification status.",
    href: "/sales",
  },
  {
    title: "Convert to deal and project",
    detail: "Move won demand into delivery and assign ownership.",
    href: "/projects",
  },
  {
    title: "Log delivery costs",
    detail: "Track time and expense signals tied to active work.",
    href: "/time",
  },
  {
    title: "Invoice and close cash",
    detail: "Issue invoices, send, and mark paid for closeout.",
    href: "/invoicing",
  },
] as const;
...
export default function WorkspacePage() {
  ...
  return (
    ...
    >
      <section
        className={`card overflow-hidden border ${demoMode ? "border-emerald-300 bg-gradient-to-r from-emerald-50 via-white to-cyan-50" : "border-border bg-white"}`}
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted">Guided Demo</p>
            <h2 className="mt-1 text-lg font-semibold tracking-tight text-ink">Lead-to-cash walkthrough</h2>
            <p className="mt-1 max-w-2xl text-sm leading-6 text-muted">
              Follow this sequence for investor demos: pipeline capture, conversion, delivery tracking, and invoice closeout.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {demoMode ? (
              ...
              <Link href="/workspace?demo=1" className="btn">
                Start demo mode
              </Link>
            )}
          </div>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
          {DEMO_STEPS.map((step, index) => (
            <Link
              key={step.title}
              href={step.href}
              className={`rounded-md border px-3 py-3 text-sm transition hover:-translate-y-0.5 ${demoMode ? "border-emerald-200 bg-white" : "border-border bg-muted"}`}
            >
              <p className="num text-xs uppercase tracking-[0.08em] text-muted">Step 0{index + 1}</p>
